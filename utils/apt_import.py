...
# Import functions based on https://github.com/oscarbranson/apt-tools

def get_dpos(URL_pos, URL_rrng, output_features=['id', 'x', 'y', 'z', 'comp', 'Da', 'element', 'n'],dissolve_n=False):

    """
    Load and process files in one function
    """

    pos = read_epos(URL_pos)
    ions, rrngs = read_rrng(URL_rrng)
    lpos = label_ions(pos, rrngs)
    dpos = deconvolve(lpos)
    dpos = dpos.reset_index(drop=True)

    dpos["n"] = pd.to_numeric(dpos["n"], downcast="integer")

    if dissolve_n:
        print("dissolve_n active") 
        dpos = dpos.loc[dpos.index.repeat(dpos['n'])].copy()
        dpos['n'] = 1

    dpos.insert(0, "id", np.arange(len(dpos)))
    dpos = dpos[output_features]
...
def read_pos(f):
    ...
    d = data.byteswap().newbyteorder()  # force native byteorder
    # unpack data
    pos = pd.DataFrame({"x": d[0::4], "y": d[1::4], "z": d[2::4], "Da": d[3::4]})
    return pos
...
def read_epos(f):
    ...
    Columns:
        ...
        pslep: Pulses since last event pulse (i.e. ionization rate)
...
def read_rrng(f):
    ...
    rrngs = pd.DataFrame(
        rrngs, columns=["number", "lower", "upper", "vol", "comp", "color"]
    )
    ...
    rrngs.drop_duplicates(inplace=True)
...
def label_ions(pos, rrngs):
    ...
    with composition and Color, based on an imported .rrng file."""

    pos["comp"] = ""
    pos["color"] = "#FFFFFF"

    for n, r in rrngs.iterrows():
        pos.loc[(pos.Da >= r.lower) & (pos.Da <= r.upper), ["comp", "color"]] = [
            r["comp"],
            "#" + r["color"],
        ]
